...
//判断两圆是否相交或相切
bool cal_distance(const Circle &c1, const Circle &c2) {
    double point_distance = sqrt(pow(c1.getX() - c2.getX(), 2) + pow(c1.getY() - c2.getY(), 2));
    double sum_of_r = c1.getR() + c2.getR();
    if (point_distance <= sum_of_r) return true;
    else return false;
}
...
vector<Circle> func8(const vector<Circle> &circles) {
    ...
    for (int i = 0; i < circles.size(); ++i) {
        ...
        for (int j = 0; j < circles.size(); ++j) {
            if (i == j) continue;
            if (cal_distance(circles[i], circles[j])) flag = false;
        }
        if (flag) v.push_back(circles[i]);
    }
    //完成排序
    sort(v.begin(), v.end(), [](const Circle &c1, const Circle &c2) {
        if (c1.getArea() != c2.getArea()) return c1.getArea() > c2.getArea();
...
int main() {
    ...
    cout << "======Test func8===========\n";
    vector<Circle> vec1 = {
        Circle(0, 0, 1), Circle(0, 2, 2), Circle(0, 3, 2),
        Circle(1, 2, 1), Circle(2, 2, 1), Circle(3, 3, 1), Circle(6, 7, 2)
    };
    cout << func8(vec1) << endl;
    vector<Circle> vec2 = {
        Circle(0, 0, 1), Circle(3, 4, 1), Circle(-2, -3, 2),
